...
class Pagecontrol extends CI_Controller {
	...
	function insertOP_in(){
		...
		$data = array(
			...
			'no_container_in' => $this->input->post('no_transaksi_in'),
	...
	function cetak(){
        ...
        $object->getActiveSheet()->getStyle("J1:L1")->applyFromArray(
            ...
            $object->getActiveSheet()->getStyle("Y1:Z1")->applyFromArray(
                array(
                    'fill' => array(
                        'type' => PHPExcel_Style_Fill::FILL_SOLID,
                        'color' => array('rgb' => '#CD5C5C')
            ...
            $object->getActiveSheet()->getStyle("AH1:AI1")->applyFromArray(
                array(
                    'fill' => array(
                        'type' => PHPExcel_Style_Fill::FILL_SOLID,
                        'color' => array('rgb' => '#CD5C5C')
            ...
            //OP dan DOK OP stop
            $object->getActiveSheet()->getStyle("AJ1:CT1")->applyFromArray(
                array(
                    'fill' => array(
                        'type' => PHPExcel_Style_Fill::FILL_SOLID,
            ...
            //AP stop
            $object->getActiveSheet()->getStyle("CU1:EC1")->applyFromArray(
                array(
                    'fill' => array(
                        'type' => PHPExcel_Style_Fill::FILL_SOLID,
            ...
            //AR stop
            $object->getActiveSheet()->getStyle("ED1:EP1")->applyFromArray(
                array(
                    'fill' => array(
                        'type' => PHPExcel_Style_Fill::FILL_SOLID,
            ...
            //Truck stop
            $object->getActiveSheet()->getStyle("EQ1:EW1")->applyFromArray(
                array(
                    'fill' => array(
                        'type' => PHPExcel_Style_Fill::FILL_SOLID,
            ...
            //pettycash stop
            $object->getActiveSheet()->getStyle("EX1:FF1")->applyFromArray(
                array(
                    'fill' => array(
                        'type' => PHPExcel_Style_Fill::FILL_SOLID,
		...
		$table_columns = array("No Transaksi" , "IMO" , "No. & Size Container" , "Name CUST" , "No. Shipment 1" , "Tanggal Kirim Dokumen No. Shipment 1 ke AR" , "Tanggal Terima Dokumen Shipment 1" , "Tanggal Kirim Dokumen Shipment 1 ke AR" , "No. Shipment 2" , "Tanggal Kirim Dokumen No. Shipment 2 ke AR" , "Tanggal Terima Dokumen Shipment 2" , "Tanggal Kirim Dokumen Shipment 2 ke AR" , "No. Seal" , "Full / Empty" , "Reefer / Dry" , "Stuffing Date" , "Container In / Out" , "Origin Town" , "Destination Town" , "Vessel Name" , "Delivery From JKT (ETD)" , "Arv At Dest (ETA)" , "Tanggal Dooring Container" , "No. Shipment ULI HUB 1" , "Tanggal Terima Dokumen Shipment ULI HUB 1" ,  "Tanggal Kirim Dokumen Shipment ULI HUB 1" , "Tujuan Shipment ULI HUB 1" , "Tgl Dooring Shipment ULI HUB 1" , "No. Shipment ULI HUB 2" , "Tanggal Terima Dokumen Shipment ULI HUB 2" , "Tanggal Kirim Dokumen Shipment ULI HUB 2" , "Tujuan Shipment ULI HUB 2" , "Tgl Dooring Shipment ULI HUB 2" , "Tanggal Terima Dokumen Plugging" , "Tanggal Kirim Dokumen Plugging Ke AR" ,
			//23 & 4
			"Name AGEN" ,"Invoice No. AGEN" ,"Invoice AGEN date" ,"Tgl Terima Invoice" ,"Invoice AGEN Amount" ,"Payment amount AGEN" ,"Payment Date AGEN" , "Invoice No. GENSET" , "Invoice GENSET date" , "Tgl Terima Invoice" , "Invoice GENSET Amount" , "Payment GENSET" , "Payment Date GENSET" , "Invoice No. BJTI" , "Invoice BJTI date" , "Tgl Terima Invoice" , "Invoice BJTI Amount" , "Payment BJTI" , "Payment Date BJTI" , "Name SHIP" , "Invoice No. SHIP" , "Invoice SHIP date" , "Tgl Terima Invoice" , "Invoice SHIP Amount" , "Payment SHIP" , "Payment Date SHIP" , "Invoice No. THC" , "Invoice THC date" , "Tgl Terima Invoice" , "Invoice THC Amount" , "Payment THC" , "Payment Date THC" , "Invoice No. PLUG" , "Invoice PLUG date" , "Tgl Terima Invoice" , "Invoice PLUG Amount" , "Payment PLUG" , "Payment Date PLUG" ,  "Tanggal Kirim Dokumen Plugging" ,"Invoice No. Buruh" , "Invoice Buruh date" , "Tgl Terima Invoice" , "Invoice Buruh Amount" , "Payment Buruh" , "Payment Date Buruh" , "No. Invoice Shipment ULI HUB 1" , "Tgl Invoice Shipment ULI HUB 1" , "Tgl Terima Invoice" , "Invoice Shipment ULI HUB 1 Amount" , "Payment Shipment ULI HUB 1" , "Payment Date Shipment ULI HUB 1" , "No. Invoice Shipment ULI HUB 2" , "Tgl Invoice Shipment ULI HUB 2" , "Tgl Terima Invoice" , "Payment Date Shipment ULI HUB 2 Amount" , "Payment Shipment ULI HUB 2" , "Payment Date Shipment ULI HUB 2" , "Invoice No. LAIN" , "Invoice LAIN date" , "Tgl Terima Invoice" , "Invoice LAIN Amount" , "Payment LAIN" , "Payment Date LAIN" ,
			//62
		...
		foreach($AllData as $row)
		{
			...
			$object->getActiveSheet()->setCellValueByColumnAndRow(25, $excel_row, $row->tgl_kirim_dok_uli );

			$object->getActiveSheet()->setCellValueByColumnAndRow(26, $excel_row, $row->tuj_ship_uli);
			$object->getActiveSheet()->setCellValueByColumnAndRow(27, $excel_row, $row->tgl_door_uli);
			$object->getActiveSheet()->setCellValueByColumnAndRow(28, $excel_row, $row->no_ship_uli2);

			$object->getActiveSheet()->setCellValueByColumnAndRow(29, $excel_row, $row->tgl_dok_uli2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(30, $excel_row, $row->tgl_kirim_dok_uli2);

			$object->getActiveSheet()->setCellValueByColumnAndRow(31, $excel_row, $row->tuj_ship_uli2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(32, $excel_row, $row->tgl_door_uli2);

			$object->getActiveSheet()->setCellValueByColumnAndRow(33, $excel_row, $row->tgl_dok_plugging);
			$object->getActiveSheet()->setCellValueByColumnAndRow(34, $excel_row, $row->tgl_kirim_dok_plugging);
			//OP
			$object->getActiveSheet()->setCellValueByColumnAndRow(35, $excel_row, $row->name_ag);
			$object->getActiveSheet()->setCellValueByColumnAndRow(36, $excel_row, $row->inv_ag);
			$object->getActiveSheet()->setCellValueByColumnAndRow(37, $excel_row, $row->inv_ag_date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(38, $excel_row, $row->tgl_trima_inv_ag);
			$object->getActiveSheet()->setCellValueByColumnAndRow(39, $excel_row, $row->inv_ag_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(40, $excel_row, $row->pay_ag);
			$object->getActiveSheet()->setCellValueByColumnAndRow(41, $excel_row, $row->date_ag);
			$object->getActiveSheet()->setCellValueByColumnAndRow(42, $excel_row, $row->inv_genset);
			$object->getActiveSheet()->setCellValueByColumnAndRow(43, $excel_row, $row->inv_genset_date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(44, $excel_row, $row->tgl_trima_inv_genset);
			$object->getActiveSheet()->setCellValueByColumnAndRow(45, $excel_row, $row->inv_genset_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(46, $excel_row, $row->pay_genset);
			$object->getActiveSheet()->setCellValueByColumnAndRow(47, $excel_row, $row->date_genset);
			$object->getActiveSheet()->setCellValueByColumnAndRow(48, $excel_row, $row->inv_bjti);
			$object->getActiveSheet()->setCellValueByColumnAndRow(49, $excel_row, $row->inv_date_bjti);
			$object->getActiveSheet()->setCellValueByColumnAndRow(50, $excel_row, $row->tgl_trima_inv_bjti);
			$object->getActiveSheet()->setCellValueByColumnAndRow(51, $excel_row, $row->inv_bjti_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(52, $excel_row, $row->pay_bjti);
			$object->getActiveSheet()->setCellValueByColumnAndRow(53, $excel_row, $row->date_bjti);
			$object->getActiveSheet()->setCellValueByColumnAndRow(54, $excel_row, $row->name_ship);
			$object->getActiveSheet()->setCellValueByColumnAndRow(55, $excel_row, $row->inv_ship);
			$object->getActiveSheet()->setCellValueByColumnAndRow(56, $excel_row, $row->inv_ship_date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(57, $excel_row, $row->tgl_trima_inv_ship);
			$object->getActiveSheet()->setCellValueByColumnAndRow(58, $excel_row, $row->inv_ship_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(59, $excel_row, $row->pay_ship);
			$object->getActiveSheet()->setCellValueByColumnAndRow(60, $excel_row, $row->date_ship);
			$object->getActiveSheet()->setCellValueByColumnAndRow(61, $excel_row, $row->inv_thc);
			$object->getActiveSheet()->setCellValueByColumnAndRow(62, $excel_row, $row->inv_thc_date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(63, $excel_row, $row->tgl_trima_inv_thc);
			$object->getActiveSheet()->setCellValueByColumnAndRow(64, $excel_row, $row->inv_thc_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(65, $excel_row, $row->pay_thc);
			$object->getActiveSheet()->setCellValueByColumnAndRow(66, $excel_row, $row->date_thc);
			$object->getActiveSheet()->setCellValueByColumnAndRow(67, $excel_row, $row->inv_plug);
			$object->getActiveSheet()->setCellValueByColumnAndRow(68, $excel_row, $row->inv_plug_date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(69, $excel_row, $row->tgl_trima_inv_plug);
			$object->getActiveSheet()->setCellValueByColumnAndRow(70, $excel_row, $row->inv_plug_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(71, $excel_row, $row->pay_plug);
			$object->getActiveSheet()->setCellValueByColumnAndRow(72, $excel_row, $row->date_plug);

			$object->getActiveSheet()->setCellValueByColumnAndRow(73, $excel_row, $row->tgl_kirim_dok_ar);

			$object->getActiveSheet()->setCellValueByColumnAndRow(74, $excel_row, $row->inv_buruh);
			$object->getActiveSheet()->setCellValueByColumnAndRow(75, $excel_row, $row->inv_buruh_date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(76, $excel_row, $row->tgl_trima_inv_buruh);
			$object->getActiveSheet()->setCellValueByColumnAndRow(77, $excel_row, $row->inv_buruh_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(78, $excel_row, $row->pay_buruh);
			$object->getActiveSheet()->setCellValueByColumnAndRow(79, $excel_row, $row->date_buruh);
			$object->getActiveSheet()->setCellValueByColumnAndRow(80, $excel_row, $row->inv_uli);
			$object->getActiveSheet()->setCellValueByColumnAndRow(81, $excel_row, $row->tgl_inv_uli);
			$object->getActiveSheet()->setCellValueByColumnAndRow(82, $excel_row, $row->tgl_trima_uli);
			$object->getActiveSheet()->setCellValueByColumnAndRow(83, $excel_row, $row->inv_uli_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(84, $excel_row, $row->pay_ship_uli);
			$object->getActiveSheet()->setCellValueByColumnAndRow(85, $excel_row, $row->pay_date_uli);
			$object->getActiveSheet()->setCellValueByColumnAndRow(86, $excel_row, $row->inv_uli2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(87, $excel_row, $row->tgl_inv_uli2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(88, $excel_row, $row->tgl_trima_uli2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(89, $excel_row, $row->inv_uli_am2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(90, $excel_row, $row->pay_ship_uli2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(91, $excel_row, $row->pay_date_uli2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(92, $excel_row, $row->inv_lain);
			$object->getActiveSheet()->setCellValueByColumnAndRow(93, $excel_row, $row->inv_lain_date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(94, $excel_row, $row->tgl_trima_inv_lain);
			$object->getActiveSheet()->setCellValueByColumnAndRow(95, $excel_row, $row->inv_lain_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(96, $excel_row, $row->pay_lain);
			$object->getActiveSheet()->setCellValueByColumnAndRow(97, $excel_row, $row->date_lain);
			//AP
			$object->getActiveSheet()->setCellValueByColumnAndRow(98, $excel_row, $row->no_inv_ship);
			$object->getActiveSheet()->setCellValueByColumnAndRow(99, $excel_row, $row->tgl_dok_ship);
			$object->getActiveSheet()->setCellValueByColumnAndRow(100, $excel_row, $row->tgl_inv_ship);
			$object->getActiveSheet()->setCellValueByColumnAndRow(101, $excel_row, $row->inv_ship_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(102, $excel_row, $row->no_faktur_ship);
			$object->getActiveSheet()->setCellValueByColumnAndRow(103, $excel_row, $row->tgl_inv_ship_pay);
			$object->getActiveSheet()->setCellValueByColumnAndRow(104, $excel_row, $row->inv_ship_paid_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(105, $excel_row, $row->no_inv_ship2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(106, $excel_row, $row->tgl_dok_ship2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(107, $excel_row, $row->tgl_inv_ship2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(108, $excel_row, $row->inv_ship_am2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(109, $excel_row, $row->no_faktur_ship2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(110, $excel_row, $row->tgl_inv_ship_pay2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(111, $excel_row, $row->inv_ship_paid_am2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(112, $excel_row, $row->no_inv_uli);
			$object->getActiveSheet()->setCellValueByColumnAndRow(113, $excel_row, $row->tgl_dok_uli);
			$object->getActiveSheet()->setCellValueByColumnAndRow(114, $excel_row, $row->tgl_inv_uli); 
			$object->getActiveSheet()->setCellValueByColumnAndRow(115, $excel_row, $row->inv_uli_am);
			$object->getActiveSheet()->setCellValueByColumnAndRow(116, $excel_row, $row->no_faktur_uli); 
			$object->getActiveSheet()->setCellValueByColumnAndRow(117, $excel_row, $row->tgl_inv_uli_pay);
			$object->getActiveSheet()->setCellValueByColumnAndRow(118, $excel_row, $row->inv_uli_paid_am); 
			$object->getActiveSheet()->setCellValueByColumnAndRow(119, $excel_row, $row->no_inv_uli2); 
			$object->getActiveSheet()->setCellValueByColumnAndRow(120, $excel_row, $row->tgl_dok_uli2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(121, $excel_row, $row->tgl_inv_uli2); 
			$object->getActiveSheet()->setCellValueByColumnAndRow(122, $excel_row, $row->inv_uli_am2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(123, $excel_row, $row->no_faktur_uli2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(124, $excel_row, $row->tgl_inv_uli_pay2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(125, $excel_row, $row->inv_uli_paid_am2);
			$object->getActiveSheet()->setCellValueByColumnAndRow(126, $excel_row, $row->no_plug);
			$object->getActiveSheet()->setCellValueByColumnAndRow(127, $excel_row, $row->inv_plugar_date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(128, $excel_row, $row->tgl_dok_plug);
			$object->getActiveSheet()->setCellValueByColumnAndRow(129, $excel_row, $row->inv_plugar_am );
			$object->getActiveSheet()->setCellValueByColumnAndRow(130, $excel_row, $row->plug_pay_date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(131, $excel_row, $row->pay_plug_paid);
			$object->getActiveSheet()->setCellValueByColumnAndRow(132, $excel_row, $row->no_faktur);
			//AR
			$object->getActiveSheet()->setCellValueByColumnAndRow(132, $excel_row, $row->inv_truck);
			$object->getActiveSheet()->setCellValueByColumnAndRow(133, $excel_row, $row->name);
			$object->getActiveSheet()->setCellValueByColumnAndRow(134, $excel_row, $row->date);
			$object->getActiveSheet()->setCellValueByColumnAndRow(135, $excel_row, $row->tujuan);
			$object->getActiveSheet()->setCellValueByColumnAndRow(136, $excel_row, $row->pesanan);
			$object->getActiveSheet()->setCellValueByColumnAndRow(137, $excel_row, $row->no_pol);
			$object->getActiveSheet()->setCellValueByColumnAndRow(138, $excel_row, $row->jam);
			$object->getActiveSheet()->setCellValueByColumnAndRow(139, $excel_row, $row->muatan);
			$object->getActiveSheet()->setCellValueByColumnAndRow(140, $excel_row, $row->ukuran);
			$object->getActiveSheet()->setCellValueByColumnAndRow(141, $excel_row, $row->b_jajan);
			$object->getActiveSheet()->setCellValueByColumnAndRow(142, $excel_row, $row->b_kom);
			$object->getActiveSheet()->setCellValueByColumnAndRow(143, $excel_row, $row->b_kawal);
			$object->getActiveSheet()->setCellValueByColumnAndRow(144, $excel_row, $row->b_lain);
			//TRUCK
			$object->getActiveSheet()->setCellValueByColumnAndRow(145, $excel_row, $row->b_stuff);
			$object->getActiveSheet()->setCellValueByColumnAndRow(146, $excel_row, $row->plus_b_stuff);
			$object->getActiveSheet()->setCellValueByColumnAndRow(147, $excel_row, $row->electricson);
			$object->getActiveSheet()->setCellValueByColumnAndRow(148, $excel_row, $row->b_karantina);
			$object->getActiveSheet()->setCellValueByColumnAndRow(149, $excel_row, $row->b_handlfull);
			$object->getActiveSheet()->setCellValueByColumnAndRow(149, $excel_row, $row->b_lolo);
			$object->getActiveSheet()->setCellValueByColumnAndRow(150, $excel_row, $row->b_adm);
			$object->getActiveSheet()->setCellValueByColumnAndRow(151, $excel_row, $row->b_lain);
			//PETTY CASH
			$object->getActiveSheet()->setCellValueByColumnAndRow(152, $excel_row, $row->full_emptymc);
			$object->getActiveSheet()->setCellValueByColumnAndRow(153, $excel_row, $row->cust_balik);
			$object->getActiveSheet()->setCellValueByColumnAndRow(154, $excel_row, $row->no_sealmc);
			$object->getActiveSheet()->setCellValueByColumnAndRow(155, $excel_row, $row->tuj_balik);
			$object->getActiveSheet()->setCellValueByColumnAndRow(156, $excel_row, $row->kapal_balik);
			$object->getActiveSheet()->setCellValueByColumnAndRow(157, $excel_row, $row->stuff_date_balik);
			$object->getActiveSheet()->setCellValueByColumnAndRow(158, $excel_row, $row->ETD_balik);
			$object->getActiveSheet()->setCellValueByColumnAndRow(159, $excel_row, $row->ETA_balik);
			$object->getActiveSheet()->setCellValueByColumnAndRow(160, $excel_row, $row->tgl_dooring);
			//MONITORING CONTAINER
			$excel_row++;
		}
